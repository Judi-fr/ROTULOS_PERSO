...
Son las mismas que históricamente vivían como enum. Se marcan con
``es_sistema`` para que ni la API ni el admin las dejen borrar: hay
...
importa rótulos desde una foto (apps.processing.agente) para decidir a qué
variable corresponde cada dato detectado.

Idempotente: usa get_or_create, así reaplicarla no duplica ni pisa cambios
que alguien haya hecho después desde el admin.
"""
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("labels", "0005_plantilla_variablerotulo_elementoplantilla"),
    ]
