...
ocurre una sola vez en ``resolution.py``, y los backends quedan tontos:
...
izquierda del rótulo, igual que en ``LayoutElement``. Los backends que
...
class Text:
    ...
    La caja (``width_mm`` × ``height_mm``) no recorta: sirve para alinear
    horizontalmente y para centrar verticalmente. El texto ya viene truncado
    por ``resolution`` si no entraba, y ``truncated`` deja constancia de eso.
    """
    ...
    # Código de familia (ver apps.labels.element_layout_render.fonts). None = la por
...
class Image:
    ...
    ``UploadedLabelFile``, un código QR generado, y un código de barras
    generado. Que los tres lleguen acá como bytes de PNG es lo que evita que
    los backends tengan que saber nada de códigos ni de archivos.
...
class Line:
    ...
    Se describe por su caja igual que todo lo demás: si ``height_mm`` es
    menor que ``width_mm`` es horizontal, y al revés vertical. Se dibuja por
    el centro de la caja, así que el grosor crece hacia ambos lados.
    """
...
class Canvas:
    ...
    - ``missing``: variables de la plantilla que no vinieron en los datos.
    ...
    - ``truncated``: textos que no entraban en su caja y se cortaron. Cortar
    ...
    - ``warnings``: el resto de los problemas que no impiden imprimir pero sí
