...
def generate_qr(value):
    """Devuelve ``(png, modules)`` con el código QR de ``value``.

    ``modules`` es cuántos cuadraditos tiene el código de lado, borde
    incluido. Lo devuelve porque quien lo pega en el rótulo es el único que
    sabe de qué tamaño físico va a quedar la caja, y con las dos cosas puede
    calcular cuánto mide cada módulo impreso — que es lo que decide si un
    escáner lo va a poder leer. Sin ese dato, meter un envío entero adentro
    de un QR de dos centímetros produce un código perfecto e ilegible.
    ...
    return buffer.getvalue(), qr.modules_count + BORDER_MODULES * 2


def generate_barcode(value):
    """Devuelve un PNG con el código de barras de ``value``, o ``None``.
    ...
    try:
        ...
        logger.warning("No se pudo generar el código de barras de %r", value)
        return None
