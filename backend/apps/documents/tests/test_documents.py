...
from ..models import MAX_BYTES_IMAGEN, UploadedLabelFile, detectar_tipo
...
class SubidaDeDocumentosTests(APITestCase):
    ...
    def test_requiere_autenticacion(self):
        respuesta = self.client.post(self.url, {"file": archivo("r.png", PNG)})
        self.assertEqual(respuesta.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_sube_una_imagen(self):
        self.client.force_authenticate(self.usuario)
        respuesta = self.client.post(
            self.url, {"file": archivo("rotulo.png", PNG)}, format="multipart"
        )
        self.assertEqual(respuesta.status_code, status.HTTP_201_CREATED)
        self.assertEqual(respuesta.data["mime_type"], "image/png")
        self.assertEqual(respuesta.data["original_filename"], "rotulo.png")
        self.assertEqual(respuesta.data["uploaded_by"], self.usuario.pk)
    ...
    def test_el_tipo_sale_del_contenido_y_no_de_la_peticion(self):
        ...
        respuesta = self.client.post(
            self.url,
            {"file": archivo("virus.png", b"MZ\x90\x00...", "image/png")},
            format="multipart",
        )
        self.assertEqual(respuesta.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("file", respuesta.data)

    def test_un_jpeg_disfrazado_de_png_se_guarda_como_jpeg(self):
        self.client.force_authenticate(self.usuario)
        respuesta = self.client.post(
            self.url,
            {"file": archivo("foto.png", JPEG, "image/png")},
            format="multipart",
        )
        self.assertEqual(respuesta.status_code, status.HTTP_201_CREATED)
        self.assertEqual(respuesta.data["mime_type"], "image/jpeg")

    def test_una_imagen_demasiado_grande_da_400(self):
        self.client.force_authenticate(self.usuario)
        gigante = PNG + b"\x00" * (MAX_BYTES_IMAGEN + 1)
        respuesta = self.client.post(
            self.url, {"file": archivo("grande.png", gigante)}, format="multipart"
        )
        self.assertEqual(respuesta.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_acepta_pdf(self):
        ...
        respuesta = self.client.post(
            self.url,
            {"file": archivo("rotulo.pdf", PDF, "application/pdf")},
            format="multipart",
        )
        self.assertEqual(respuesta.status_code, status.HTTP_201_CREATED)
        self.assertEqual(respuesta.data["mime_type"], "application/pdf")
        self.assertFalse(UploadedLabelFile.objects.get(pk=respuesta.data["id"]).is_image)

    def test_cada_usuario_solo_ve_sus_documentos(self):
        UploadedLabelFile.objects.create(
            file="rotulos/a.png",
    ...
    def test_borrar_un_documento_deja_auditlog(self):
        ...
        propio = UploadedLabelFile.objects.create(
            file="rotulos/b.png",
        ...
        self.assertFalse(UploadedLabelFile.objects.filter(pk=propio.pk).exists())
