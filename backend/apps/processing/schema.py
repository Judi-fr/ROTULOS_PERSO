...
Este módulo es el otro extremo del contrato que ``apps.labels.styles`` dejó
...
De ahí también que ``LayoutVariable.description`` sea funcional y no
...
Las claves de este esquema son el "idioma" en el que Claude responde: no
están atadas 1:1 a ningún campo guardado (lo que se guarda en
``LayoutElement.style`` usa sus propias claves en español, ver
``apps.labels.styles`` — ``agent.py`` es el traductor entre ambos
vocabularios).
"""

from apps.labels.styles import ALIGNMENTS

# Tipos de elemento que el modelo puede proponer. Es el mismo conjunto de
# `apps.labels.models.LayoutElementType`, pero se declara acá como literales
# porque viaja dentro de un JSON Schema y no como choices de Django.
ELEMENT_TYPES = ("variable", "texto_estatico", "linea", "recuadro")
...
def _enum_or_null(values, description):
    """Campo que es uno de ``values`` o ``null``, en la forma que acepta la API.

    Lo natural sería ``{"type": ["string", "null"], "enum": values + [None]}``,
...
def build_schema(variables):
    ...
    ``variables`` es un iterable de ``LayoutVariable``. Sus códigos forman el
    ...
    return {
        ...
        "properties": {
            ...
            "elements": {
                ...
                "items": _element_schema(codes),
            },
        },
    }


def _element_schema(codes):
    ...
    return {
        ...
        "properties": {
            ...
            "variable": _enum_or_null(
                codes,
                "Solo cuando element_type es 'variable': el código del "
                "catálogo que corresponde a este dato. null en cualquier "
                "otro caso. Si un dato variable no encaja en ninguno de los "
                "códigos disponibles, no lo fuerces: marcá el elemento como "
                "texto_estatico y explicá en 'notes' qué campo faltaría.",
            ),
            "content": {
                "type": ["string", "null"],
                "description": (
                    "Solo cuando element_type es 'texto_estatico': el texto "
                    "literal tal como está impreso. null en cualquier otro "
                    "caso."
                ),
            },
            "detected_value": {
                "type": ["string", "null"],
                "description": (
                    "Solo cuando element_type es 'variable': el valor "
                    "concreto que se lee en ESTA foto (por ejemplo el "
                    "nombre real del destinatario). No forma parte de la "
                    "plantilla — sirve para que quien revisa verifique que "
                    "el campo se identificó bien. null en cualquier otro "
                    "caso."
                ),
            },
            "x_pct": {
...
def build_system_prompt(variables):
    ...
    """
    catalog = "\n".join(
        f"- `{v.code}` ({v.get_data_type_display()}) — {v.label}: "
        f"{v.description or 'sin descripción'}"
        for v in variables
    )
...
marcalo como `texto_estatico` y aclaralo en `notes`, así un administrador puede \
...
bajá `confidence` y explicá en `notes` qué no pudiste leer."""
